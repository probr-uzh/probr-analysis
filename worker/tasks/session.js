  ...
  RawSession.find().sort("-value.endTimestamp").limit(1).exec(function (err, rawSessionDocs) {
    ...
    if (rawSessionDocs[0] !== undefined) {
      ...
      Packet.mapReduce(SessionMapReduce.getIncrementalMapReduceConfig(rawSessionBreakTime), function (err, results) {
        ...
        Session.find().sort("-endTimestamp").limit(1).exec(function (err, sessionDocs) {
          var sessionBreakTime = sessionDocs[0].endTimestamp;
          console.log("my session breaker "+sessionBreakTime)
          SessionForEach.incremental(sessionBreakTime, function () {
